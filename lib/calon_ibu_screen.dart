...
import 'package:stunting/input_ibu_screen.dart';
...
import 'calon_ibu_form_screen.dart';

class CalonIbuScreen extends StatefulWidget {
  const CalonIbuScreen({Key? key}) : super(key: key);

  @override
  State<CalonIbuScreen> createState() => _CalonIbuScreenState();
}

class _CalonIbuScreenState extends State<CalonIbuScreen> {
  ...
  void searching(String search) async {
    ...
    if (searchController.text.isNotEmpty) {
      var users = await PesertaService().getIbu(search: searchController.text);
      setState(() {
        peserta = users;
        isLoading = false;
      });
            ...
            GestureDetector(
              onTap: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: ((context) => InputIbuScreen()),
                  ),
                );
              },
              child: Container(
                      ...
                      itemBuilder: (context, index) => PesertaListItem(
                        ...
                        press: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) =>
                                  CalonIbuFormScreen(ibu: peserta[index]),
                            ),
                          );
                        },
                      ),
